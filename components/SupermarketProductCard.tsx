  ...
  return (
    <div className="card" style={{ border: "1px solid #e5e7eb", padding: 12, borderRadius: 8 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <button onClick={() => addItem({ id: product.id, name: product.name, price: product.price, qty: 1, image: product.image, stock: product.stock })} style={{ flex: 1, padding: "8px 12px" }}>
          Add to cart
        </button>
        <Link href={`/supermarket/product/${product.id}`}>
