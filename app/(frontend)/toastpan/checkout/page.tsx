...
import { useAuth } from '../../supermarket/contexts/AuthContext';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { getAddresses } from '@/app/actions/addresses';
import { Address } from '../../supermarket/types/types';
import { createOrder } from '@/app/actions/orders';
import { useOrders } from '../../supermarket/contexts/OrderContext';
...
export default function ToastPanCheckoutPage(): React.ReactNode {
    const { getCartItems, getCartTotal, clear } = useCart();
    const { user } = useAuth();
    const { refreshOrders } = useOrders();
    const router = useRouter();
    const storeFilter = 'toastpan';
    ...
    if (!user) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#fcfbf9]">
                <p>Please <Link href="/supermarket/account/login" className="text-[#B5D04E] font-bold">login</Link> to continue.</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#fcfbf9]">
                <div className="text-center">
                    <p className="mb-4">Your Toast Pan cart is empty.</p>
    ...
    const handlePaystackSuccess = async (reference: PaystackReference) => {
        ...
        try {
            ...
            if (result.success && result.order) {
                clear(storeFilter);
                await refreshOrders();
                router.push(`/toastpan/order-confirmation?orderId=${result.order.id}`);
            } else {
        ...
        <div className="min-h-screen bg-[#fcfbf9] py-8 px-4">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-black">Toast Pan Checkout</h1>
                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                            ...
                            {userAddresses.length === 0 ? (
                                <div className="text-center py-8">
                                    ...
                                    <Link href="/supermarket/account/addresses" className="text-[#B5D04E] hover:text-[#A3BC46] font-medium">Add Address →</Link>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {userAddresses.map(address => (
                                        <div
                                            ...
                                            className={`border-2 rounded-xl p-4 cursor-pointer transition-all ${selectedAddress?.id === address.id ? 'border-[#B5D04E] bg-[#B5D04E]/5' : 'border-gray-100 hover:border-gray-200'}`}
                                        >
                                            <div className="flex items-start justify-between">
                                                ...
                                                {address.isDefault && <span className="bg-[#B5D04E] text-white text-[10px] px-2 py-0.5 rounded-full uppercase font-bold">Default</span>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                            ...
                            <div className="space-y-3">
                                <div
                                    ...
                                    className={`border-2 rounded-xl p-4 cursor-pointer transition-all ${paymentMethod === 'paystack' ? 'border-[#B5D04E] bg-[#B5D04E]/5' : 'border-gray-100 hover:border-gray-200'}`}
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-[#B5D04E]/10 rounded-lg flex items-center justify-center text-2xl">💳</div>
                                        <div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24 border border-gray-100">
                                ...
                                {items.map(item => (
                                    <div key={item.id} className="flex gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[#B5D04E] font-bold text-sm mt-1">₦{(item.price * item.qty).toLocaleString()}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="border-t border-gray-100 pt-4 mb-8">
                                <div className="flex justify-between text-2xl font-black">
                                    ...
                                    <span className="text-[#B5D04E]">₦{orderTotal.toLocaleString()}</span>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className={`w-full text-white py-4 rounded-xl font-bold text-lg mb-4 transition-all shadow-xl ${!selectedAddress || processing || !storeId ? 'bg-gray-200 cursor-not-allowed text-gray-400 shadow-none' : 'bg-[#B5D04E] hover:bg-[#A3BC46] shadow-[#B5D04E]/20 active:scale-[0.98]'}`}
                            >
